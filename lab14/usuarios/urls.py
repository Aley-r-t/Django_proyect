
from django.urls import path
from rest_framework_simplejwt.views import TokenRefreshView
from .views import ListaUsuariosView, CustomTokenObtainPairView, DetalleUsuarioView, CrearUsuarioView, EliminarUsuarioView

urlpatterns = [
    path('login/', CustomTokenObtainPairView.as_view(), name='token_obtain_pair'),
    path('refresh/', TokenRefreshView.as_view(), name='token_refresh'),
    path('lista/', ListaUsuariosView.as_view(), name='lista_usuarios'),
    path('usuario/<int:pk>/', DetalleUsuarioView.as_view(), name='detalle_usuario'),
    path('usuario/crear/', CrearUsuarioView.as_view(), name='crear_usuario'),
    path('usuario/eliminar/<int:pk>/', EliminarUsuarioView.as_view(), name='eliminar_usuario'),
]